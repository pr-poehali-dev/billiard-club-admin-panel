...
const BOOKINGS_API = 'https://functions.poehali.dev/d3a3df18-5aa2-42a4-80c2-51e735c1e1ac';
...
const Tables = () => {
  ...
  useEffect(() => {
    fetch(`${SETTINGS_API}?resource=tables`)
      .then((r) => r.json())
      .then((d) => {
      ...
      .catch(() => toast.error('Не удалось загрузить столы'))
  ...
  const table = tables.find((t) => t.id === selectedTable) ?? null;
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {tablesLoading ? (
            ...
            <Card className="p-8 rounded-3xl border-border/60 text-center text-muted-foreground text-sm">
              Нет столов. Добавьте их в «Настройки столов».
            </Card>
          ) : (
            tables.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-primary">{t.price_per_hour}₽</p>
        ...
        <Card className="lg:col-span-3 p-7 rounded-3xl border-border/60 shadow-sm h-fit space-y-7">
          {table ? (
            <>
              <div>
                <h2 className="font-sans text-2xl font-bold mb-1">{table.name}</h2>
                <p className="text-sm text-muted-foreground">{table.table_type} · {table.size_ft} фт · {table.price_per_hour}₽/час</p>
              </div>
            ...
            {bookings.map((b) => (
              <div key={b.id} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-muted/50 transition-colors group">
                ...
                <button
                  ...
                  className="w-9 h-9 rounded-lg flex items-center justify-center text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors opacity-0 group-hover:opacity-100"
                >
