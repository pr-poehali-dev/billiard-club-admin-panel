...
const emptyNew = () => ({
  ...
  model: '', description: '', controller_id: '',
});
...
const TableSettings = () => {
  ...
  const startEdit = (t: BilliardTable) => {
    setEditingId(t.id);
    setEditData({
      name: t.name, table_type: t.table_type, size_ft: t.size_ft,
      price_per_hour: t.price_per_hour, model: t.model,
      description: t.description, controller_id: t.controller_id,
    });
  };
    ...
    <div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <Input value={data.name || ''} onChange={(e) => onChange('name', e.target.value as BilliardTable['name'])} className="rounded-xl h-10" placeholder="Стол №1" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Модель стола</Label>
          <Input value={data.model || ''} onChange={(e) => onChange('model', e.target.value as BilliardTable['model'])} className="rounded-xl h-10" placeholder="Brunswick Gold Crown VI" />
        </div>
      </div>
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="space-y-1">
          <Label className="text-xs">Тип игры</Label>
          <Select value={data.table_type} onValueChange={(v) => onChange('table_type', v as BilliardTable['table_type'])}>
        ...
        <div className="space-y-1">
          ...
          <Select value={String(data.size_ft)} onValueChange={(v) => onChange('size_ft', Number(v) as BilliardTable['size_ft'])}>
            <SelectTrigger className="rounded-xl h-10"><SelectValue /></SelectTrigger>
        ...
        <div className="space-y-1">
          ...
          <Input type="number" value={data.price_per_hour || ''} onChange={(e) => onChange('price_per_hour', Number(e.target.value) as BilliardTable['price_per_hour'])} className="rounded-xl h-10" />
        </div>
      </div>
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="space-y-1">
          <Label className="text-xs">ID контроллера</Label>
          <Input value={data.controller_id || ''} onChange={(e) => onChange('controller_id', e.target.value as BilliardTable['controller_id'])} className="rounded-xl h-10 font-mono" placeholder="CTRL-001" />
        </div>
        <div className="space-y-1 sm:row-span-2">
          <Label className="text-xs">Описание</Label>
          <Textarea value={data.description || ''} onChange={(e) => onChange('description', e.target.value as BilliardTable['description'])} className="rounded-xl resize-none" rows={3} placeholder="Дополнительная информация о столе" />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <AdminLayout
      ...
      actions={
        <Button onClick={() => { setAdding(true); setEditingId(null); }} size="lg" className="rounded-xl gap-2">
        ...
        {adding && (
          <Card className="p-6 rounded-3xl border-primary/30 bg-primary/5 shadow-sm animate-scale-in">
            ...
            <div className="flex gap-2 mt-5">
              ...
              <Button onClick={() => { setAdding(false); setNewTable(emptyNew()); }} variant="outline" size="sm" className="rounded-xl">Отмена</Button>
            </div>
          </Card>
        )}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {t.model && ` · ${t.model}`}
                        {t.controller_id && <span className="ml-2 font-mono opacity-60">{t.controller_id}</span>}
                      </p>
