...
def handler(event: dict, context) -> dict:
    '''Управление бронированиями столов: получение списка, создание брони'''
    ...
    if method == 'GET':
        cur.execute(
            ...
            "time_slot, client_name, status FROM bookings "
        ...
        items = [{
            ...
            'time_slot': r[4], 'client_name': r[5], 'status': r[6],
        } for r in rows]
    ...
    cur.execute(
        f"INSERT INTO bookings (table_id, table_name, booking_date, time_slot, client_name, status) "
        f"VALUES ({table_id}, '{table_name}', '{date}', '{time_slot}', '{client_name}', '{status}') "
        f"RETURNING id"
    )
